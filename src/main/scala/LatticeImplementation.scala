...
package LatticeImplementation {
  ...
  case class Environment(
                          interestRate: BinomialLatticeBounded[Double],
  ...
  object LatticeImplementation {
    ...
    def binomialValuation[A](pr: PROpt[A], marketData: Environment): BinomialLattice[A] = pr match {
      ...
      case Snell(date: LocalDate, c: PROpt[Double]) => {
        ...
        val _process: BinomialLatticeBounded[Double] = new PassThroughBoundedBL[Double]((date: LocalDate) => (idx: Int) => con(date)(idx), daysUntilMaturity + 1)
        //daysUntilMaturity+1 because if contract matures today, it still needs today's valuation
        var result = _process
