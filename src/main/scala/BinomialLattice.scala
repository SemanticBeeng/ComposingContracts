...
//may need to 'grow' lattice on-demand, else just use Array
...
package BinomialLattice {
  ...
  trait BinomialLattice[A] {
    ...
    //probability is assumed to be uniform in this implementation (in other words, ignored)
    def startVal(): A = apply(0)(0)

    def zip[B](lattice: BinomialLattice[B]): BinomialLattice[(A, B)] = {
      //map is probably a better name for this
      new BinomialLattice[(A, B)] {
        override def apply(i: Int): RandomVariable[(A, B)] = (j: Int) => (BinomialLattice.this (i)(j), lattice(i)(j))
      }
    }

    def map[B](func: (A) => B): BinomialLattice[B] = {
      //map is probably a better name for this
  ...
  trait BinomialLatticeBounded[A] extends BinomialLattice[A] {
    ...
    override def zip[B](lattice: BinomialLattice[B]): BinomialLatticeBounded[(A, B)] = {
      //map is probably a better name for this
    ...
    override def map[B](func: (A) => B): BinomialLatticeBounded[B] = {
      //map is probably a better name for this
